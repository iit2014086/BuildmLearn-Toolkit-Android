...
public class InfoTest {
    ...
    public final ActivityTestRule<TemplateEditor> mActivityRule =
            new ActivityTestRule<TemplateEditor>(TemplateEditor.class) {
                ...
                protected Intent getActivityIntent() {
                    ...
                    result.putExtra(Constants.TEMPLATE_ID, 0);
                    return result;
                }
            };
    ...
    private void toolbarTitle() {
        String title = "Info Template";
        onView(allOf(instanceOf(TextView.class), withParent(withId(R.id.toolbar))))
                .check(matches(withText(title)));
    }

    private void addInfos() {
        String word = "Test Info";
        String description = "Description for testing info";
    ...
    private void addTemplate() {
        ...
        onView(withId(R.id.template_title)).perform(replaceText("Testing Info Template"));
    }
    ...
    private void checkSimulator() {
        ...
        onView(withText("Testing Info Template")).check(matches(isDisplayed()));
        sleep();
        ...
        onView(allOf(instanceOf(TextView.class), withParent(withId(R.id.toolbar_main)), withText("Testing Info Template")))
                .check(matches(isDisplayed()));
        sleep();

        onData(anything()).inAdapterView(withId(R.id.list_view_info)).atPosition(0).perform(click());
        sleep();

        onView(withId(R.id.title)).check(matches(withText(word))).check(matches(isDisplayed()));
        onView(withId(R.id.description)).check(matches(withText(description))).check(matches(isDisplayed()));
        sleep();
